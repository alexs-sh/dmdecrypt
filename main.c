/*
 * Based on dmcryptgen:
 * https://source.denx.de/denx/meta-secure-imx/-/blob/dunfell/recipes-crypto/dmcryptgen/files/dmcrypt_gen.c
 *
...
static ssize_t do_decrypt(const EVP_CIPHER *cipher,
			...
			void *plaintext)
{
	...
	if (EVP_DecryptUpdate(ctx, plaintext, &len, ciphertext, size + iv_len) != 1)
		goto fail_ctx;

	if (cipher == EVP_aes_256_xts()) {
	fprintf(stderr,"AES 256 XTS not supported!");
	return -1;
	}
...
static ssize_t decrypt_sector(const void *ciphertext, size_t size, const void *key,
			      uint64_t sector, void *plaintext,
			      const EVP_CIPHER *cipher)
{
	...
	if ((cipher == EVP_aes_128_cbc()) || (cipher == EVP_aes_256_cbc())) {
		tweak = alloca(ivsize);
	...
	return do_decrypt(cipher, ciphertext, SECTOR_SIZE, key, key_len, tweak,
			  ivsize, plaintext);
}
...
int main(int argc, char *argv[])
{
	...
	ret = strlen(argv[2]);
	if (!(ret == 32 || ret == 64)) {
		fprintf(stderr, "Invalid key length. Not 32 or 64 bytes!\n");
		return 1;
	}

	ret = hex2bin(key, argv[2], 32);
	if (ret == -1) {
	...
	for (sector = 0; sector < total_sectors; sector++) {
		ret = pread(fd, crbuf, SECTOR_SIZE, SECTOR_SIZE * sector);
		if (ret != SECTOR_SIZE) {
		...
		ret = decrypt_sector(crbuf, SECTOR_SIZE, key, sector, buf, EVP_aes_256_cbc());
		if (ret < 0) {
		...
		ret = pwrite(fd, buf, SECTOR_SIZE, SECTOR_SIZE * sector);
		if (ret != SECTOR_SIZE) {
